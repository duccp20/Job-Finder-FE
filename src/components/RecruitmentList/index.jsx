...
const RecruitmentList = () => {
  ...
  useEffect(() => {
    const fetchData = async (apiCallFunction) => {
      const no = Math.min(currentPage, totalPages);
      try {
        const result = await apiCallFunction(no, 3, selectedOption);
    ...
    if (selectedOption === "Đang mở") {
      fetchData(callGetActiveJobByCompanyID);
    } else if (selectedOption === "Đã đóng") {
      fetchData(callGetDisableJobByCompanyID);
    } else {
      fetchData(callGetAllJobByCompanyID);
    }
    ...
    <div className="h-auto w-full">
      ...
      <div className="mb-[30px] flex w-full justify-between gap-[13px] ">
        ...
        <div className="flex w-[45%] gap-[13px]">
          ...
          <div className="relative flex w-[33%] cursor-pointer ">
            <div className="z-[2] flex items-center justify-between rounded border px-[13px] py-[12px]">
              ...
              <div className="relative flex">
                <div className=" z-[2] flex cursor-pointer items-center justify-between">
                  <input
                    type="text"
                    name="deadline"
                    id="deadline"
                ...
                <td className="border px-[18px] py-[15px]">
                  <div>
                    ...
                    <p className="text-base font-normal not-italic text-[#333333]">
                      Tạo lúc: {item.startDate}
                    </p>
                    <p className="text-base font-normal not-italic text-[#333333]">
                      Cập nhật lúc: {item.lastModifiedDate}
                    </p>
                    <p className="text-base font-normal not-italic text-[#333333]">
                      Địa điểm làm việc: {item.province}
                    </p>
                ...
                <td className="border px-[18px] py-[15px] text-center">
                  <div
                    className={`w-[90px] py-[9px] text-center ${
                      item.statusDTO.name === "Active"
                        ? "bg-[#88D4987A]"
                        : "bg-[#FF00005C]"
                    } rounded-[5px]`}
                  >
                    {item.statusDTO.name === "Active" ? "Đang mở" : "Đã đóng"}
                  </div>
                </td>
                <td className="border px-[18px] py-[15px] text-center">
                  <div>
